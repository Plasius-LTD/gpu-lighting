# ADR-0009: Hosted OIDC Package Publication
...
`@plasius/gpu-lighting` must publish without a long-lived npm write token and without
...
newer, request provenance, and prohibit npm write-token fallbacks. CI uses
explicit GitHub-hosted runners, and fork PR code is denied.
