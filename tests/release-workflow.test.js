...
test("uses exact-main hosted OIDC publication without write tokens", () => {
  ...
  assert.match(cdWorkflow, /successful exact-SHA main CI/u);
  ...
  assert.equal(cdWorkflow.match(/npm@11\.6\.2/gu)?.length, 2);
  assert.match(cdWorkflow, /"11\.5\.1"/u);
  assert.match(cdWorkflow, /Revalidate exact main immediately before npm publication/u);
  assert.match(cdWorkflow, /Checked-out release commit/u);
  assert.match(cdWorkflow, /--provenance/u);
  assert.doesNotMatch(cdWorkflow, /NPM_TOKEN|NODE_AUTH_TOKEN/u);
});

test("keeps pull-request CI on explicit trusted hosted runners", () => {
  assert.match(ciWorkflow, /workflow_dispatch:/u);
  assert.match(ciWorkflow, /pull_request:/u);
  assert.match(ciWorkflow, /Trusted head admission/u);
